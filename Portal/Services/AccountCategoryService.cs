...
public sealed class AccountCategoryService
{
    ...
    public AccountCategoryService(string connectionString) => _cs = connectionString;

    public async Task<List<AccountCategory>> GetAllAsync(CancellationToken ct = default)
    {
        const string sql = "SELECT [CategoryID],[Name],[Description],[IsActive],[CreatedAt] FROM [dbo].[AccountCategories] ORDER BY [Name];";
    ...
    public async Task<AccountCategory?> GetByIdAsync(int id, CancellationToken ct = default)
    {
        const string sql = "SELECT [CategoryID],[Name],[Description],[IsActive],[CreatedAt] FROM [dbo].[AccountCategories] WHERE [CategoryID]=@ID;";
        await using var conn = new SqlConnection(_cs);
        await conn.OpenAsync(ct);
        await using var cmd = new SqlCommand(sql, conn);
    ...
    /// <summary>Gets existing category by name, or creates it. Returns the ID.</summary>
    public async Task<int> EnsureAsync(string name, CancellationToken ct = default)
    {
        name = name.Trim();
        const string getSql = "SELECT [CategoryID] FROM [dbo].[AccountCategories] WHERE [Name]=@Name;";
        await using var conn = new SqlConnection(_cs);
        await conn.OpenAsync(ct);
        ...
        const string insSql = "INSERT INTO [dbo].[AccountCategories]([Name],[Description]) OUTPUT INSERTED.[CategoryID] VALUES(@Name,'');";
        await using var insCmd = new SqlCommand(insSql, conn);
        insCmd.Parameters.AddWithValue("@Name", name);
        return (int)(await insCmd.ExecuteScalarAsync(ct))!;
    }

    public async Task<int> CreateAsync(AccountCategory item, CancellationToken ct = default)
    {
        const string sql = "INSERT INTO [dbo].[AccountCategories]([Name],[Description]) OUTPUT INSERTED.[CategoryID] VALUES(@Name,@Desc);";
        await using var conn = new SqlConnection(_cs);
        await conn.OpenAsync(ct);
        await using var cmd = new SqlCommand(sql, conn);
    ...
    public async Task UpdateAsync(AccountCategory item, CancellationToken ct = default)
    {
        const string sql = "UPDATE [dbo].[AccountCategories] SET [Name]=@Name,[Description]=@Desc WHERE [CategoryID]=@ID;";
        await using var conn = new SqlConnection(_cs);
        await conn.OpenAsync(ct);
        await using var cmd = new SqlCommand(sql, conn);
    ...
    {
        const string sql = "DELETE FROM [dbo].[AccountCategories] WHERE [CategoryID]=@ID;";
        await using var conn = new SqlConnection(_cs);
        await conn.OpenAsync(ct);
        await using var cmd = new SqlCommand(sql, conn);
        cmd.Parameters.AddWithValue("@ID", id);
    ...
    private async Task UpsertRowAsync(string name, string description, ImportResult result, CancellationToken ct)
    {
        ...
        await using var conn = new SqlConnection(_cs);
        await conn.OpenAsync(ct);
        await using var chk = new SqlCommand("SELECT [CategoryID] FROM [dbo].[AccountCategories] WHERE [Name]=@Name;", conn);
        chk.Parameters.AddWithValue("@Name", name.Trim());
        var existing = await chk.ExecuteScalarAsync(ct);

        if (existing is int id)
        {
            await using var upd = new SqlCommand("UPDATE [dbo].[AccountCategories] SET [Description]=@Desc WHERE [CategoryID]=@ID;", conn);
        ...
        {
            await using var ins = new SqlCommand("INSERT INTO [dbo].[AccountCategories]([Name],[Description]) VALUES(@Name,@Desc);", conn);
    ...
    private static AccountCategory Map(SqlDataReader r) => new()
    {
